...
describe('<Home />', () => {
  ...
  it('should render search, posts and load more', async () => {
    render(<Home />);
    const noMore = screen.getByText('Não exist Posts!');

    expect.assertions(3);

    await waitForElementToBeRemoved(noMore);
  ...
  it('should search for posts', async () => {
    ...
    expect(screen.getByText('Não exist Posts!')).toBeInTheDocument();
  });
...
// await waitForElementToBeRemoved(noMorePosts); -> Fala par esperar sumir, e como é assíncrono temos que colocar await, ea função tem que ser async
